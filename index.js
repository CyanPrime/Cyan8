...
chip8.prototype.loadGame = function(myGame){
	...
	myGameArray = new Uint8Array(myGame);
...
chip8.prototype.cycle = function(){
	...
	switch(this.opcode & 0xF000){
		case 0x0000:
			...
			{ 
				case 0x00E0: // 0x00E0: Clears the screen 
					if(printCall) console.log("00E0: clear the screen");
					for(var x = 0; x < 64; x++){
						for(var y = 0; y < 32; y++){
							this.gfx[(y * 64) + x] = 0;
						}
				...
				case 0x00EE: // 0x00EE: Returns from subroutine 
					if(printCall) console.log("00EE: Returns from subroutine");
		...
		case 0x1000:
			if(printCall) console.log("1000: jump to addr = " + (this.opcode & 0x0FFF));
		...
		case 0x2000:
			if(printCall) console.log("2000: Calls subroutine at = " + (this.opcode & 0x0FFF));
		...
		case 0x3000: // 0x3XNN: Skips the next instruction if VX equals NN
			if(printCall) console.log("0XNN - skip if equal: " + this.V[x] + " = " + (this.opcode & 0x00FF));
			if(this.V[x] == (this.opcode & 0x00FF))
				this.state = 2;
		break;
		
		case 0x4000: // 0x4XNN: Skips the next instruction if VX doesn't equals NN
			if(printCall) console.log("4XNN - skip if not equal: " + this.V[x] + " = " + (this.opcode & 0x00FF));
			if(this.V[x] != (this.opcode & 0x00FF))
				this.state = 2;
		break;
		
		case 0x5000: // 0x5XNN: Skips the next instruction if VX equals VY
			if(printCall) console.log("5XNN - skip if equal: " + this.V[x] + " = " + this.V[y]);
		...
		//V[X] = NN;
		case 0x6000:
			if(printCall) console.log("6XNN - set V[X] to NN: this.V[x](" + this.V[x] + ") = " + (this.opcode & 0x00FF));
		...
		case 0x7000:
			...
			if(printCall) console.log("7XNN -  V[X] += NN: this.V[x](" + this.V[x] + ") = " + val);
			this.V[x] = val;
		break;
		...
		case 0x8000:
			switch((this.opcode & 0x000f)){
				...
				case 0x0000:
					if(printCall) console.log("8XY0 - Set VX to VY");
					this.V[x] = this.V[y];
				break;
				...
				//Sets VX to VX OR VY.
				case 0x0001:
					if(printCall) console.log("8XY1 - Set VX to VX OR VY");
					this.V[x] |= this.V[y];
				break;
				...
				case 0x0002:
					if(printCall) console.log("8XY2 - Set VX to VX AND VY");
					this.V[x] &= this.V[y];
				break;
				...
				case 0x0003:
					if(printCall) console.log("8XY3 - Set VX to VX XOR VY");
					this.V[x] ^= this.V[y];
				break;
				...
				case 0x0004:
					if(printCall) console.log("8XY4 - Adds VY to VX. VF is set to 1 when there's a carry, and to 0 when there isn't.");
				...
				case 0x0005:
					if(printCall) console.log("8XY5 - VY is subtracted from VX. VF is set to 0 when there's a borrow, and 1 when there isn't.");
					if(this.V[x] > this.V[y])
				...
				case 0x0006:
					if(printCall) console.log("8XY6 - Shifts VX right by one. VF is set to the value of the least significant bit of VX before the shift.");
				...
				case 0x0007:
					if(printCall) console.log("8XY7 - Sets VX to VY minus VX. VF is set to 0 when there's a borrow, and 1 when there isn't.");
		...
		case 0xD000:
			if(printCall) console.log("DXYN - Draw screen.");
		...
		case 0xE000:
			switch((this.opcode & 0x00FF)){
				...
				case 0x00A1:
					if(printCall) console.log("EXA1: key = " + this.key[this.V[x]]);
					if(this.key[this.V[x]] == 0){
...
app.get('/', function(req, res){
  res.sendFile(__dirname + '/gamec.html');
});
...
io.on('connection', function (socket) {
	...
	socket.on('keydown', function(msg){
		if(socket.id == playerOne || socket.id == playerTwo){
			myChip8.key[myChip8.keyMap[msg]] = true;
			myChip8.currentKey = myChip8.keyMap[msg];
		}
		//console.log('keydown');
	});
	...
	socket.on('keyup', function(msg){
		if(socket.id == playerOne || socket.id == playerTwo){
			myChip8.key[myChip8.keyMap[msg]] = false;
...
var drawGraphics = function(){
	...
	for(var x = 0; x < 64; x++){
		for(var y = 0; y < 32; y++){
			if(myChip8.gfx[x + (y * 64)]) screenArray[y][x] = 1;
		}
	}
